 ...
 * @param     {function}  express  Express instance
 *
 * @return    {Function}
 */
module.exports.listen = function(sol, express) {
  ...
  _.each(sol.config.http.middleware.order, function(option) {
    try {
      var fn = new require(sol.paths.api + '/middleware/' + option)(sol);
      app.use(fn);
      ...
      if (apiErr.code !== 'MODULE_NOT_FOUND') {
        ...
        try {
          var fn = new require('./middleware/' + option)(sol);
          app.use(fn);
  ...
  orm(sol, function(err, models) {
     ...
     * Start listening
     */
    app.listen(sol.port || 3001);
    sol.log.info('Listening on ' + (sol.port || 3001)  + ' (' + sol.env + ')');
  });
};
